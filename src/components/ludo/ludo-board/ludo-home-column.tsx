    ...
    return <div className={clsx("ludo-home__column", customClassName)}>
        <div className="ludo-token__container">
            <div className="ludo-token__grid">
                {token?.sort((a, b) => a.sn - b.sn).map((data) => <LudoToken {...data} isInHomeColumn homeActive={!data?.position} active={data?.active} key={`${color}-${data.sn}`} color={color} onClick={() => handleTokenClick(data)} />)}
            </div>
        </div>
    </div>
}
